 ...
 */
async function rollback(store, key, options, result) {
  if (typeof store.rollbackTokenBucket === 'function') {
    await store.rollbackTokenBucket(key, result._internal?.rollbackData);
    return;
  }

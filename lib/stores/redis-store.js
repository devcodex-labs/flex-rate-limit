 ...
 * @param {boolean} options.ownsClient - Whether this store should close the Redis client
 */
  constructor(options = {}) {
    if (!options.client) {
    ...
    this.ownsClient = options.ownsClient === true || options.closeClient === true;
    this.sequence = 0;
    this.instanceId = options.instanceId || `${INSTANCE_ID}-${(storeInstanceSequence += 1).toString(36)}`;
  ...
  _createMember(timestamp) {
    ...
    return `${timestamp}-${this.instanceId}-${this.sequence.toString(36)}`;
  }
  ...
  /**
   * Close the Redis client when this store owns it.
   * External clients remain caller-owned by default.
   * @returns {Promise<void>}
   */
  async close() {
    if (!this.ownsClient) {

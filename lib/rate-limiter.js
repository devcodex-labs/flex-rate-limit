...
class RateLimiter {
  ...
  _initializeStore(store) {
    ...
    if (typeof store === 'string' && store.startsWith('redis://')) {
      ...
      return new RedisStore({ client, ownsClient: true });
    }
  ...
  async _checkSlow(key, options) {
    ...
    const attachInternal = options.trackRollback === true;

    return this._checkResolved(key, effectiveConfig, algorithm, algorithmOptions, attachInternal);
  }

  async _checkResolved(key, effectiveConfig, algorithm, algorithmOptions, attachInternal = true) {
    try {
  ...
  /**
   * Close owned store resources, such as Redis clients created from connection strings.
   * @returns {Promise<void>}
   */
  async close() {
   ...
   */
  middleware(_options = {}) {
    const hasOverrides = Object.keys(_options).length > 0;
    const canUseStaticPath = !hasOverrides && this._hasStaticMax && !this._hasRouteConfig;

    return async (req, res, next) => {
      const route = req.route?.path || req.path || req.url;
      const config = canUseStaticPath ? this.options : this._resolveConfig(route, _options);

      try {
        // 检查是否应跳过速率限制
        const skipResult = config.skip(req);
        const shouldSkip = skipResult && typeof skipResult.then === 'function'
          ? await skipResult
          : skipResult;
        if (shouldSkip) {
        ...
        // 生成速率限制键（传递路由上下文）
        const keyResult = config.keyGenerator(req, { route });
        const key = keyResult && typeof keyResult.then === 'function'
          ? await keyResult
          : keyResult;

        // 检查速率限制（传递路由信息）
        const shouldRollback = config.skipSuccessfulRequests || config.skipFailedRequests;
        const result = canUseStaticPath
          ? await this._checkResolved(
            key,
            config,
            this.algorithm,
            this._baseAlgorithmOptions,
            shouldRollback,
          )
        ...
        const postProcess = shouldRollback ? this._createPostProcess(result, config, res) : null;
        const registerPostProcessBeforeNext = !!(
          shouldRollback
          && res
          && typeof res.once === 'function'
        );

        // Register response lifecycle listeners before next(), because a synchronous handler
        // can finish the response before middleware() regains control.
        if (registerPostProcessBeforeNext) {
          postProcess();
        }
        ...
        if (nextResult && typeof nextResult.then === 'function') {
          ...
          if (shouldRollback && !registerPostProcessBeforeNext) {
            await postProcess();
          }
          return output;
        }

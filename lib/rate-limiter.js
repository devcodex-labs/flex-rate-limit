...
class RateLimiter {
  ...
  async check(key, options = {}) {
    ...
    const effectiveConfig = options.config || this._resolveConfig(options.route, options.overrides);
    const max = await this._resolveMax(effectiveConfig, options.req);
    const algorithmConfig = this._buildAlgorithmOptions(
      { ...effectiveConfig, max },
      options,
    );
    const algorithm = this._getAlgorithm(effectiveConfig.algorithm);
    ...
    try {
      ...
      return {
        allowed: true,
        limit: algorithmConfig.limit,
        current: 0,
        remaining: algorithmConfig.limit,
